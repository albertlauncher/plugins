...
Plugin::Plugin() : d(make_unique<Private>())
{
    ...
    case CNAuthorizationStatusRestricted: {
        const char *m = QT_TR_NOOP("Access to contacts is restricted.");
        WARN << m;
        throw runtime_error(tr(m).toStdString());
    }
    case CNAuthorizationStatusDenied: {
        const char *m = QT_TR_NOOP("Access to contacts is denied.");
        WARN << m;
        throw runtime_error(tr(m).toStdString());
    }
    case CNAuthorizationStatusNotDetermined: {
        ...
        if (!granted)
            throw runtime_error(tr("User denied access to contacts.").toStdString());
